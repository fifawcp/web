...
export default function LoginPage() {
  ...
  const step: LoginStep =
    stepParam === "otp" || stepParam === "identifier" ? stepParam : "identifier";
...
function LoginPageShell({
  ...
  return (
    <div className="mx-auto w-full max-w-md">
      <StepIndicator
        steps={["step_identifier", "step_verify"].map((s) => t(s))}
        currentStep={stepIndex}
      />
      <Card className="bg-card">{children}</Card>
      <p className="mt-6 text-center text-sm text-muted-foreground">
        {t("noAccount")} <Link href="/register" className="font-medium text-foreground hover:underline">{t("createOne")}</Link>
      </p>
    </div>
  );
}
