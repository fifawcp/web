...
export function TeamRow({ row, locale, groupPicks }: Props) {
    ...
    <tr className="border-t border-border first:border-t-0">
      ...
      <td className="py-2.5 pr-2">
        <div className="flex items-center gap-2">
          <Image src={row.team.flag_url} alt="" width={20} height={14} className="h-3.5 w-5 shrink-0 rounded-sm object-cover" unoptimized />
          <span className="truncate text-sm font-medium">{teamName}</span>
          <span className="hidden text-2xs text-muted-foreground sm:inline">{row.team.fifa_code}</span>
        </div>
      </td>
      ...
      <td className="px-2 py-2.5 text-right text-sm font-semibold tabular-nums">{row.points}</td>
      {comparison && (
        <>
          <td className="px-2 py-2.5 text-center">
            {comparison.predicted_position !== null ? (
              <span
                className={cn(
          ...
          <td className="px-2 py-2.5 text-center text-xs tabular-nums">
            {comparison.delta == null ? (
              ...
              <span className="text-muted-foreground">↓{comparison.delta}</span>
            ) : (
              <span className="text-muted-foreground">↑{Math.abs(comparison.delta)}</span>
            )}
          </td>
        </>
      )}
